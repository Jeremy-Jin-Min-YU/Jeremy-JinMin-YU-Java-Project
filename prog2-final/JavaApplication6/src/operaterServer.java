...
{  
   ...
   public PassClass getRouteTable() throws Exception
   {
       ...
       s.execute ("CALL operator2.getRouteCity()");
       ...
       pc = new PassClass ("route","operator2");  // pack up for send
       pc.al_route = set;
       return pc;
   }
   ...
   {
       PassClass bookResult = new PassClass("bookResult","operator2");

        Boolean hadResult = false;
	int book_id = 0;
       try {
               CallableStatement book = conn.prepareCall("{CALL operator2.insert_book(?)}");
   ...
   public synchronized void cancelTicket(PassClass pc)
   {
         try {
             int id = pc.bookId;
               CallableStatement cancel = conn.prepareCall("{CALL operator2.delete_book(?)}");
   ...
   public  PassClass searchTicket(PassClass pc)
   {
	   ...
	   for (int i = 0;i<pc.al_sp.size();i++)
	   {
		   ...
		   try {
			   CallableStatement getTicket = conn.prepareCall("{call operator2.search_ticket(?,?,?)}");
	   ...
	   searchResult = new PassClass ("SearchResult","operator2");  // pack up for send
